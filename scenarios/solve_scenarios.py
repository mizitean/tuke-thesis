...
def solve_network(
    ...
    if "slack" in n.generators.carrier.values:
        ...
        if slack_gens.isin(n.generators_t.p.columns).any():
            slack_p = n.generators_t.p[slack_gens.intersection(n.generators_t.p.columns)]
...
def _record(results_df: pd.DataFrame, path: Path, row: pd.Series, result: dict) -> pd.DataFrame:
    record = {**dict(row), **result}
    new_row = pd.DataFrame([record])
    if "tag" in results_df.columns and record.get("tag") in results_df["tag"].values:
        results_df.loc[results_df["tag"] == record["tag"], list(result.keys())] = list(result.values())
    else:
...
def run(
    ...
    for _, row in manifest.iterrows():
        ...
        if not net_path.exists():
            print(f"  MISSING  {tag}")
            results_df = _record(results_df, results_path, row, {"status": "missing_input"})
            failed += 1
            continue
    ...
    with multiprocessing.Pool(processes=workers, maxtasksperchild=1) as pool:
        ...
        for task, ar in async_results:
            ...
            if status == "solved":
                solved += 1
                slack_warn = f"  ⚠ slack={slack:.0f} MWh" if slack and slack > 1000 else ""
                obj_str = f"  obj={obj:.0f}" if obj is not None else ""
                print(f"  [{total_done}/{total}] OK       {result['tag']}  {t}s{obj_str}{slack_warn}")
            else:
                failed += 1
                print(f"  [{total_done}/{total}] FAILED   {result['tag']}  status={status}")

    elapsed = time.time() - t_start
    print(f"\n{'='*60}")
    print(f"Done in {elapsed/60:.1f} min  —  {solved} solved, {skipped} skipped, {failed} failed")
